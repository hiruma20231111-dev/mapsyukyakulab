...
export function mintToken({ geminiKey, label = "", days = 14, model = "gemini-2.5-flash" }) {
  ...
  const data = JSON.stringify({ gk: geminiKey, l: String(label).slice(0, 40), e: exp, m: model, c: Date.now() });
...
export function verifyToken(token) {
  ...
  try {
    ...
    return { valid: !expired, expired, label: p.l, exp: p.e, gk: p.gk, model: p.m };
  } catch {
    return null;
  }
}
