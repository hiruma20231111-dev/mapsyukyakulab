...
export async function POST(request) {
  ...
  if (mode === "diagnose") {
    ...
    uq += `\n\n【出力ルール】\n- 点数・スコア（◯点/A〜D）は書かない。良い点と課題を「なぜそう言えるか」の根拠つきで。\n- 各セクションは必ず Markdown見出し「## 」で始める。文章は簡潔に、箇条書きは「- 」で。\n- 専門用語は使わない（DIAG_RULESの言葉づかいルールを厳守）。「レバー」という語は絶対に出力しない。\n${background ? "- 【重要】予備知識(検索で判明した公開情報)がある場合は、総評の冒頭でその具体的な値(店名・業種・★評価・クチコミ件数・サイト有無など)に必ず一度言及し、設問回答と突き合わせて講評する。ただし『検索による参考値・未確認』である旨を添える。\n" : ""}\n【セクション構成】\n## 🩺 総評\n(現在地。良い点と一番の課題を一言ずつ、なぜかの根拠つきで。予備知識があればここで具体値に言及)\n## 🎯 最優先の3手\n(順に。各手が なぜ効くか＝「集客のどの力(見つかる/選ばれる/来店/AI検索)」に効くかを、やさしい言葉で必ず添える)\n## 🛠️ 今日やる具体アクション\n(すぐできる箇条書き)\n## 🤖 AI検索（AIのおすすめ）対策\n(ChatGPT/Gemini等のAIや、GoogleのAIによる要約・おすすめに“選ばれる”ための対策を別枠で。情報の具体性・一貫性・クチコミ本文・外部での掲載がそろうほど「この店は何が良いか」がAIに伝わりやすい、を一般的傾向として。AIに聞くタイプの検索は「順位でなく選ばれるか外れるか」である点も平易に。専門語は避ける)\n## 🔗 外部サイトでの掲載（お店の実在の裏づけ）\n(HP・SNS・食べログ等ポータルで、店名・住所・電話が正しく載っている状態が、知られている度合いとAIの確信につながる旨。HP有無・SNS運用の回答をふまえ具体的に。クチコミの集め方には踏み込まない=第3条)\n## ⚠️ 注意\n(規約リスク・効果は一般的傾向で保証しない旨)`;
