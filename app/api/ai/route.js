...
export async function POST(request) {
  ...
  let { key, invite, model = "gemini-2.5-flash", dialect = "std", tone = "polite", question, diagnosis, background, history, mode, test, weakItems, bizLabel, simInfo } = b || {};
  ...
  if (mode === "diagnose") {
    ...
    const weakList = (Array.isArray(weakItems) && weakItems.length)
      ? weakItems.map((w) => `- key=${w.k} ｜ ${w.q}`).join("\n") : "";
    ...
    if (simInfo) {
      ...
      uq += `\n\n【1000人シミュレーション（データベースの予測・参考）】画面には「${simInfo.bizLabel || ""}で近くを探す1000人のうち、いまは約${simInfo.sel}人が選択」「直すと増える内訳：${liftTxt}」「ぜんぶ整えば最大 約${simInfo.ceiling}人」と表示済み。総評の中で、この“${simInfo.sel}人/1000”という現在地に軽く1文触れ、DBベースの見立てとして評価に活かす（数字は上記をそのまま使い、勝手に別の数字を作らない）。`;
    }
